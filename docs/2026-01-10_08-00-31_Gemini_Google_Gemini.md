...
title: "Google Gemini"
platform: Gemini
url: https://gemini.google.com/u/1/app/045409749ef5a5bd?pageId=none
exported: 2026-01-10T00:00:31.204Z
---
...
Thread Local Allocation Buffer 具體實作方式, 要適配 rust GC,  我們研究了 chez scheme:

Thread Local Allocation Buffer (TLAB) 在 Chez Scheme 中通過以下機制實現 thread safety：
...
使用 `alloc_mutex_acquire()` 和 `alloc_mutex_release()` 保護全局分配操作 [1](#0-0) ：
...
每個線程都有自己的 `thread_gc` 結構，包含本地分配指針 [2](#0-1) ：
...
當需要更多空間時，`S_find_more_gc_room` 函數確保線程安全 [3](#0-2) ：
...
在 GC 期間，系統會掃描所有線程的髒段並關閉線程本地分配 [4](#0-3) ：
...
對於並行垃圾回收，系統使用遠程掃描機制處理跨線程對象 [5](#0-4) ：
...
1. **分層鎖定**：日常分配使用線程本地緩衝區，無需鎖定；只有在需要新段或 GC 時才使用全局互斥鎖

2. **隔離狀態**：每個線程維護獨立的分配指針和空間計數

3. **原子操作**：關鍵狀態更新在互斥鎖保護下進行

4. **協作式 GC**：GC 期間所有線程協作，確保一致性

這種設計在保證線程安全的同时，最大程度地減少了鎖競爭，提高了分配性能。

Wiki pages you might want to explore:

- [Overview (cisco/ChezScheme)](/wiki/cisco/ChezScheme#1)

- [Runtime System (cisco/ChezScheme)](/wiki/cisco/ChezScheme#4)

### Citations

**File:** c/types.h (L412-420)
...
這是一份極具深度的技術轉換工作。我們將 **Chez Scheme** 的 C 語言實作邏輯（基於 Mutex 保護的 Global 請求 + 無鎖的 Thread Local Bump Pointer），映射到 **Rust** 的所有權模型與類型系統中。

為了符合我們先前設定的 **Plan A (BiBOP + Non-moving Mark-Sweep)**，我們的 TLAB 設計必須解決一個矛盾：**TLAB 本質上是線性的 (Bump Pointer)，但 Non-moving Heap 回收後會產生破碎的空洞 (Free List)。**

**我們的策略：**
TLAB 僅用於 **"快速路徑" (Fast Path)**，即向 Global Heap 申請一整塊 **"新鮮的/全空的 Page"** 進行線性分配。一旦 Page 滿了，或者 GC 發生，我們將該 Page "歸還" 給 Global Heap 管理（轉變為 Free List 模式或單純標記為已滿）。

以下是 **Rust 版 TLAB 實作架構**，完全對應你提供的 Chez Scheme 研究。

---

### 1. 核心數據結構 (Data Structures)

我們需要三個層次：

1. GlobalHeap : 全局鎖保護，負責派發 Pages。
2. SizeClassAllocator : 每個執行緒、每個 Size Class 一個，管理當前的 Page。
3. ThreadLocalContext : 聚合所有 Size Class 的分配器，對應 Chez 的 thread_gc 。
...
// --------------------------------------------------------
const PAGE_SIZE: usize = 4096;
const NUM_SIZE_CLASSES: usize = 32; // 例如涵蓋 8byte 到 256byte

// --------------------------------------------------------
// 1. Page & Segment (BiBOP 單元)
// --------------------------------------------------------
struct PageHeader {
    next: Option<NonNull<PageHeader>>, // Global list link
    block_size: usize,                 // Size Class
    allocated_bytes: usize,            // 用於 GC 統計
    // ... Bitmap 等其他 GC Metadata
}
...
// --------------------------------------------------------
// 2. The Global Heap (對應 Chez 的 S_alloc_mutex 保護區)
// --------------------------------------------------------
struct GlobalHeapInner {
    // 這裡存儲所有 Pages，按 Size Class 分類
    // Free Pages (完全空的頁面) 優先被 TLAB 獲取
    free_pages: [Option<NonNull<PageHeader>>; NUM_SIZE_CLASSES],
}

pub struct GlobalHeap {
    inner: Mutex<GlobalHeapInner>,
}

impl GlobalHeap {
    // 對應 Chez: S_find_more_gc_room -> alloc_mutex_acquire()
    // 這是一個 "Slow Path"，會鎖住 Mutex
    fn request_fresh_page(&self, size_class_idx: usize) -> Option<NonNull<u8>> {
        let mut guard = self.inner.lock().unwrap();
        // 1. 嘗試從 free_pages 拿
        // 2. 如果沒有，向 OS (mmap) 申請新 Page
        // 3. 初始化 PageHeader
        // 4. 返回 Page 的 data 區域起始指針
        todo!("Implement global page allocation logic")
    }

    // 當 GC 發生時，TLAB 會呼叫此函數 "歸還" 未用完的空間 (Flush)
    fn retire_page(&self, page: NonNull<PageHeader>, used_bytes: usize) {
        let mut guard = self.inner.lock().unwrap();
        unsafe {
            (*page.as_ptr()).allocated_bytes = used_bytes;
        }
        // 將 Page 加入 Global 的 Full List 或 Partial List
    }
}
```
...
### 2. TLAB 實作 (The Chez `thread_gc` Equivalent)

這是魔法發生的地方。為了達到極致性能，我們在 `AllocCursor` 中使用 Raw Pointers。

**關鍵設計：** 我們不使用 `RefCell` 來包裹每個指針，因為 `borrow_mut()` 的開銷在 `alloc` 熱點路徑上是不可接受的。我們使用 `UnsafeCell` 包裹整個 TLAB，並由開發者保證單一執行緒存取（這由 `thread_local!` 保證）。

```javascript
// 對應 Chez 的 base_loc / next_loc / bytes_left
// 但我們將其封裝為一個 Cursor
#[derive(Copy, Clone)]
struct AllocCursor {
    ptr: *mut u8,   // 當前 Bump Pointer (next_loc)
    limit: *mut u8, // 當前 Page 的結束位置 (base_loc + size)
    page_header: *mut PageHeader, // 記錄當前是哪一頁，以便 Flush
}

impl AllocCursor {
    const fn empty() -> Self {
        Self { ptr: std::ptr::null_mut(), limit: std::ptr::null_mut(), page_header: std::ptr::null_mut() }
    }
}

// 對應 Chez: thread_gc 結構體
pub struct ThreadLocalContext {
    // 針對每個 Size Class 有一個 Cursor
    cursors: [AllocCursor; NUM_SIZE_CLASSES],

    // 指向 Global Heap 的引用
    global_heap: Arc<GlobalHeap>,

    // 是否正在進行分配 (防止重入/GC 競爭)
    // 對應 Chez: tgc->during_alloc
    during_alloc: bool,
}

impl ThreadLocalContext {
    fn new(global_heap: Arc<GlobalHeap>) -> Self {
        Self {
            cursors: [AllocCursor::empty(); NUM_SIZE_CLASSES],
            global_heap,
            during_alloc: false,
        }
    }
    ...
    // ----------------------------------------------------
    #[inline(always)]
    pub unsafe fn alloc(&mut self, size_class_idx: usize, size: usize) -> *mut u8 {
        let cursor = &mut self.cursors[size_class_idx];

        // 1. 檢查剩餘空間 (Bump Pointer Check)
        // Chez: if (tgc->bytes_left[g][s] >= n) ...
        let new_ptr = cursor.ptr.add(size);

        if std::intrinsics::likely(new_ptr <= cursor.limit) {
            let res = cursor.ptr;
            cursor.ptr = new_ptr;
            return res;
        }
        ...
        // 2. 空間不足，進入慢速路徑
        self.alloc_slow(size_class_idx, size)
    }

    // ----------------------------------------------------
    // The Slow Path (Refill logic)
    // ----------------------------------------------------
    #[inline(never)] // 避免污染 Fast Path 的指令緩存
    fn alloc_slow(&mut self, size_class_idx: usize, size: usize) -> *mut u8 {
        self.during_alloc = true; // Set flag

        // 1. Close off current segment (Flush)
        // 對應 Chez: close_off_segment(...)
        self.flush_segment(size_class_idx);

        // 2. Acquire new segment from Global Heap (Locking happens here)
        // 對應 Chez: S_find_more_gc_room
        let new_page_ptr = self.global_heap.request_fresh_page(size_class_idx)
            .expect("OOM"); // Handle OOM appropriately in real GC

        // 3. Update Cursor
        // 對應 Chez: 更新 base_loc, next_loc, bytes_left
        let cursor = &mut self.cursors[size_class_idx];
        cursor.page_header = new_page_ptr.as_ptr() as *mut PageHeader; // 假設 Header 在開頭

        // 假設 data 從 header 之後開始
        let data_start = new_page_ptr.as_ptr().add(std::mem::size_of::<PageHeader>());
        let data_end = new_page_ptr.as_ptr().add(PAGE_SIZE);

        cursor.ptr = data_start;
        cursor.limit = data_end;

        self.during_alloc = false;

        // 4. Retry allocation (Recursive call, strictly once)
        unsafe { self.alloc(size_class_idx, size) }
    }

    // 對應 Chez: S_close_off_thread_local_segment
    fn flush_segment(&mut self, size_class_idx: usize) {
        let cursor = &mut self.cursors[size_class_idx];

        if !cursor.page_header.is_null() {
            // 計算用了多少
            let used = cursor.ptr as usize - cursor.page_header as usize;
            // 通知 Global Heap 更新統計資訊，並不歸還記憶體 (因為 BiBOP 頁面屬於該 SizeClass)
            self.global_heap.retire_page(
                NonNull::new(cursor.page_header).unwrap(),
                used
            );

            // 清空 Cursor
            *cursor = AllocCursor::empty();
        }
    }
}
```
...
### 3. 線程本地存儲與初始化 (Thread Initialization)

Rust 的 `thread_local!` 是 Lazy 的。我們需要在第一次存取時進行註冊。

```rust
// 全局單例 (Mock)
static GLOBAL_HEAP: once_cell::sync::Lazy<Arc<GlobalHeap>> =
    once_cell::sync::Lazy::new(|| Arc::new(GlobalHeap {
        inner: Mutex::new(GlobalHeapInner { free_pages: [None; 32] })
    }));

// Thread Local Storage
thread_local! {
    // 使用 UnsafeCell 因為我們需要內部可變性，且性能極其敏感
    static TLAB: UnsafeCell<ThreadLocalContext> = UnsafeCell::new(
        ThreadLocalContext::new(GLOBAL_HEAP.clone())
    );
}

// 公開的分配入口
pub fn gc_alloc<T>(val: T) -> *mut T {
    // 這裡需要計算 SizeClass (Trait System from previous conversation)
    let size = std::mem::size_of::<T>();
    let size_class_idx = (size + 7) / 8; // 簡單範例

    TLAB.with(|tlab_cell| {
        unsafe {
            let tlab = &mut *tlab_cell.get();
            let ptr = tlab.alloc(size_class_idx, size);
...
### 4. 與 GC 的協同 (The Handshake)

Chez Scheme 中提到 GC 發生時，會遍歷 `S_threads` 並呼叫 `S_close_off_thread_local_segment`。
在 Rust 中，我們需要一個全局的 Registry 來追蹤所有活躍的 TLAB。

**新增機制：TLAB Registry**

1. 當 ThreadLocalContext::new 被呼叫時，它必須將自己的 AllocCursor 指針（或者一個控制結構）註冊到一個全局的 Vec<Weak<ControlBlock>> 中。
2. 當 GC 觸發 (Stop-the-World) 時，GC 線程遍歷這個 Registry。
3. 關鍵點： 由於 Rust 的所有權，GC 線程不能直接修改其他線程的 AllocCursor (Data Race)。 - Chez 的做法： 依賴信號或協作檢查點。 - Rust 安全做法： 我們需要在 AllocCursor 中引入原子標記，或者要求所有線程在 alloc 的 Slow Path 或 Safe Point 自行 Flush。

**改進的協作 Flush 機制 (Poll Check):**

```css
// 在 GlobalHeap 中增加一個 Epoch 或 Flag
struct GlobalHeap {
    // ...
...
fn alloc_slow(...) {
    ...
    if self.global_heap.is_gc_requested() {
        // 主動 Flush 自己的所有 Segments
        for i in 0..NUM_SIZE_CLASSES {
            self.flush_segment(i);
        }
        // 等待 GC 完成 (Park thread)
        self.wait_for_gc();
    }
    // ... 繼續正常的 refill
}
...
1. 極速分配 (Fast Path): gc_alloc 函數在大多數情況下只有：讀取 TLS -> 檢查邊界 -> 指針加法 -> 返回。這完全對應 Chez 的無鎖設計。
2. BiBOP 適配: TLAB 被嚴格限制在 NUM_SIZE_CLASSES 的陣列中。這保證了 BiBOP 的幾何特性（Page 內無碎片）。
3. 安全性界線: 我們使用了 unsafe 和 UnsafeCell 。這在寫 Allocator 時是無法避免的。所有的不安全操作都被封裝在 ThreadLocalContext 內部。使用者拿到的將是 Gc<T> (Safe Wrapper)。
4. 借鑒 Chez: 我們保留了 during_alloc 標記（防止遞迴重入）和 flush_segment （對應 close_off_segment ）邏輯。

這就是結合了 Chez Scheme 智慧與 Rust 類型系統的 **BiBOP TLAB** 原型。
