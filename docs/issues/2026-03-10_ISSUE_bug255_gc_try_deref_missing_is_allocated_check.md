...
**Outcome:** Already fixed (duplicate of bug226).

The `Gc::try_deref` implementation in `ptr.rs` (lines 1354–1361) already includes the `is_allocated` check:
...
The fix was applied when resolving bug226. No code changes required.
