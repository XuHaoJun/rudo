...
**Invalid — already correct in tree.** `GcBox::dec_weak()` in `crates/rudo-gc/src/ptr.rs` loads `weak_count` with `Ordering::Acquire` (same as `dec_weak_raw` and `dec_ref`). The issue text referred to `Relaxed` at the loop load; that does not match current code. `cargo test -p rudo-gc --test weak --all-features -- --test-threads=1` passes. No code change required.