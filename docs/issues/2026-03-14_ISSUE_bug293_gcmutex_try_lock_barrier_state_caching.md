...
- **Component:** `GcMutex::try_lock()` in `sync.rs`
- **OS / Architecture:** All
- **Rust Version:** 1.75+
...
### 預期行為
`GcMutex::try_lock()` 應該與 `GcMutex::lock()` 一致，在獲取鎖之前緩存 barrier states，避免 TOCTOU 競爭條件。

### 實際行為
`GcMutex::try_lock()` 在 `.map()` closure 內部緩存 barrier states，這發生在鎖成功獲取之後。而 `GcMutex::lock()` 在獲取鎖之前緩存 barrier states。

這導致了不一致的行為：
- `lock()`: 緩存狀態 → 獲取鎖 → 使用緩存的狀態
- `try_lock()`: 嘗試獲取鎖 → 如果成功，緩存狀態 → 使用緩存的狀態

在 `try_lock()` 中，GC phase 在鎖獲取和狀態緩存之間可能會改變，這會導致與 `lock()` 不同的行為。
...
let guard = self.inner.lock();  // 然後獲取鎖
...
問題在於時序差異：
1. 在 `lock()` 中，狀態在鎖獲取之前緩存，確保在整個操作過程中狀態一致
2. 在 `try_lock()` 中，狀態在鎖獲取之後緩存，如果 GC phase 在這段時間內改變，會導致不一致的 barrier 行為

這與 bug290 (GcRwLock::try_write) 是相同的問題模式，但適用於 GcMutex。
...
需要精確的時序控制：
1. Thread A: 調用 `try_lock()`，開始嘗試獲取鎖
2. Thread B: GC 發生，phase 改變
3. Thread A: 獲取鎖成功，讀取新的 barrier states（與 lock() 不同的狀態）
4. 觀察到與 `lock()` 不同的 barrier 行為

---

## 🛠️ 建議修復方案 (Suggested Fix / Remediation)

將 `try_lock()` 改為與 `lock()` 一致的模式，在嘗試獲取鎖之前緩存 states：
...
- 這個問題與 bug290 (GcRwLock::try_write) 是相同模式
- Barrier states 緩存的目的是確保在整個 write 操作過程中狀態一致
- 在 `try_lock()` 中，由於緩存發生在鎖獲取之後，如果 GC phase 在此期間改變，會導致這次 write 使用不同的 barrier 配置，與 `lock()` 產生不一致的行為

**Rustacean (Soundness 觀點):**
- 這不是傳統意義上的 UB，但可能導致不正確的 GC 行為
- 如果 barrier 沒有正確觸發，可能會導致物件被錯誤地回收
