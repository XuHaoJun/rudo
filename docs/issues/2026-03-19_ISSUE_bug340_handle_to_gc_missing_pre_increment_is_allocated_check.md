...
All three locations were verified to have the fix applied in the current codebase:

1. `handles/mod.rs:373-379` — `is_allocated` pre-check exists **before** `try_inc_ref_if_nonzero`; additionally a generation check after `inc_ref` detects slot reuse (`assert_eq!(pre_generation, gc_box.generation(), ...)`).
2. `handles/async.rs:772-778` — same pattern as mod.rs.
3. `handles/cross_thread.rs:232-238` — `is_allocated` pre-check before `gc_box.inc_ref()`, with comment "Check is_allocated BEFORE inc_ref to avoid TOCTOU (bug345)"; generation check also present.

All existing handle tests pass (`test_handle_to_gc_escape_pattern`, `test_async_handle_to_gc_basic`, block_reuse suite).
