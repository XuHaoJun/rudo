...
**Outcome:** Already fixed in current `crates/rudo-gc/src/stack.rs`.

The non-x86_64 / non-aarch64 fallback uses `#[cfg(all(not(target_arch = "x86_64"), not(target_arch = "aarch64"), not(miri)))]` (see `spill_registers_and_scan`), so on **x86_64** and **aarch64** that `let regs = [0usize; 32]` block does **not** compile and cannot shadow the spilled `regs` array. Miri uses the dedicated `#[cfg(miri)] let regs = [0usize; 32];` path.

The buggy pattern described in this issue (`any(all(not(x86_64), not(aarch64)), miri)`) is **not** present in the current source.
