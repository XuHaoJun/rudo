...
**Investigation:** GcRwLockReadGuard and GcRwLockWriteGuard already had `incremental_active || generational_active` in their Drop implementations. **GcMutexGuard** at sync.rs line 716 still used only `if incremental_active`, missing the generational barrier case.

**Fix applied:** Updated GcMutexGuard::drop() to use `if incremental_active || generational_active` when calling `mark_object_black()`, matching GcThreadSafeRefMut and the RwLock guards. All sync tests pass.