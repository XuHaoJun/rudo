...
- [x] Fixed
- [ ] Regression - Bug still present in code (see below)

## 🔄 Update 2026-03-14: Regression Confirmed

The issue was marked as "Fixed" but the bug is STILL PRESENT in the code:

**Current code in `sync.rs:426-429`:**
```rust
...
The `GcRwLockReadGuard::drop()` still incorrectly calls `unified_write_barrier` when `generational_active` is true. This is a regression - the fix was either not applied or was reverted.

The issue bug261 originally described the problem with `mark_object_black` being called, but the current code ALSO has the `unified_write_barrier` call which is equally wrong.
...
Removed all barrier logic from `GcRwLockReadGuard::drop()` in `sync.rs`. Read operations do not modify data, so no SATB or generational barrier is needed. The parking_lot guard releases the read lock automatically on drop.
