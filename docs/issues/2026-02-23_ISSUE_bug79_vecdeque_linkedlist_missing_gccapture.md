...
**Outcome:** Fixed (same fix as bug67).

Added `GcCapture` for `VecDeque<T>` and `LinkedList<T>` in `cell.rs`. Regression test `bug79_vecdeque_linkedlist_missing_gccapture` passes.
