...
`Ephemeron::key()` now returns `Option<Gc<K>>` (instead of `Option<&Gc<K>>`) by delegating to `self.key.upgrade()`. Returning `Option<&Gc<K>>` is not feasible in Rust because `Weak::upgrade()` yields an owned `Gc<K>`, and we cannot return a reference to a local value. Returning `Option<Gc<K>>` is the correct semantic: the caller receives a strong reference that keeps the key alive while in use.

Tests added: `test_ephemeron_key_and_value` (asserts `key()` returns `Some` when key is alive) and `test_ephemeron_key_returns_none_when_key_dropped` (asserts `key()` returns `None` after key is collected).
