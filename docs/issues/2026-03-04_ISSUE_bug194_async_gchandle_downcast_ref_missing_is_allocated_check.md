...
Added `is_gc_box_pointer_valid` and `is_allocated` checks in `AsyncGcHandle::downcast_ref` before dereferencing `gc_box_ptr`. If the pointer is invalid or the slot was swept and reused, the function now returns `None` instead of dereferencing freed memory. Matches the pattern used in `Gc::deref`, `Weak::upgrade`, and `GcHandle::resolve`.
