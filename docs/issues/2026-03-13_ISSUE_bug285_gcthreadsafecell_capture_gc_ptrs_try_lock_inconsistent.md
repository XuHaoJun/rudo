...
Replaced `try_lock()` with blocking `lock()` in `GcThreadSafeCell::capture_gc_ptrs_into` to match `GcRwLock` and `GcMutex` behavior. This ensures all GC pointers are reliably captured when another thread holds the lock, preventing SATB invariant violations.

- **Fix:** `crates/rudo-gc/src/cell.rs` — use `self.inner.lock()` instead of `try_lock()`
- **Test:** Extended `test_gcrwlock_gcmutex_capture_gc_ptrs_into` to include `GcThreadSafeCell`
