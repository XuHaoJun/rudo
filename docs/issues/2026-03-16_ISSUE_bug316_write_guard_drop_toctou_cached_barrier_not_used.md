...
- **Component:** `GcThreadSafeRefMut`, `GcRwLockWriteGuard`, `GcMutexGuard`
- **OS / Architecture:** All
- **Rust Version:** 1.75+
...
### 預期行為
在 `borrow_mut()` 取得 guard 時，應該緩存當時的 barrier 狀態，並將這些緩存值存儲在 guard struct 中。在 Drop 時，應該使用這些緩存值來決定是否觸發 barrier，而不是重新檢查 barrier 狀態。

### 實際行為
儘管 bug160 和 bug161 聲稱已修復此問題，但當前程式碼仍然在 Drop 時重新檢查 barrier 狀態：

1. **GcThreadSafeRefMut** (`cell.rs:1372-1375`):
   - Struct 沒有存儲 `incremental_active` 或 `generational_active`
   - Drop (lines 1393-1394) 重新調用 `is_incremental_marking_active()` 和 `is_generational_barrier_active()`

2. **GcRwLockWriteGuard** (`sync.rs:429-432`):
   - Struct 沒有存儲 barrier 狀態
   - Drop (lines 460-461) 重新調用 barrier 檢查

3. **GcMutexGuard** (`sync.rs:700-703`):
   - Struct 沒有存儲 barrier 狀態
   - Drop (lines 731-732) 重新調用 barrier 檢查
...
**問題根源**: 
- `borrow_mut()` 正確緩存了 barrier 狀態 (cell.rs:1057-1058, sync.rs)
- 但緩存的值沒有存儲在 guard struct 中
...
這是經典的 TOCTOU race condition。儘管 bug160/bug161 聲稱已修復，但當前代碼仍然在 Drop 時重新檢查 barrier 狀態。這可能是修復被回滾或 regressed。需要確保 barrier 狀態在 guard acquisition 時緩存，並在 Drop 時使用。
