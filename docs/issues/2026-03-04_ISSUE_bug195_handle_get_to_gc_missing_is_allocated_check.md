...
Added `is_allocated` checks to `Handle::get()` and `Handle::to_gc()` in `crates/rudo-gc/src/handles/mod.rs`:

1. **Handle::get()**: Before dereferencing `gc_box_ptr`, added `ptr_to_object_index` + `ptr_to_page_header` + `is_allocated(idx)` check. If the slot was swept and reused, panics with "Handle::get: slot has been swept and reused".

2. **Handle::to_gc()**: (a) Same pre-dereference `is_allocated` check; (b) After `try_inc_ref_if_nonzero()`, added post-inc_ref `is_allocated` check (TOCTOU guard, same pattern as GcHandle::resolve and Gc::clone).

Behavior now matches Gc::deref, Gc::clone, and GcHandle::resolve. All HandleScope tests pass.
