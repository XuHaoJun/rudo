...
Added `is_allocated` check in `record_satb_old_value()` before recording any pointer:
- Use `ptr_to_object_index` and `ptr_to_page_header` to resolve the slot
- If `!is_allocated(idx)`, return `true` (don't record, no fallback needed)
- If `ptr_to_object_index` returns `None`, return `true` (can't resolve, don't record)

Behavior now matches other write barriers (e.g. `incremental_write_barrier`, bug286).
