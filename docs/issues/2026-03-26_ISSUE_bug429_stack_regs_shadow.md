...
**Outcome:** Already fixed in tree; no code change required.

Verified `crates/rudo-gc/src/stack.rs` `spill_registers_and_scan`:

- Fallback uses `#[cfg(all(not(target_arch = "x86_64"), not(target_arch = "aarch64"), not(miri)))]`, which is **false** on x86_64, so the `[0usize; 32]` array does not compile there and cannot shadow the spilled `regs`.
- Miri uses a separate `#[cfg(miri)] let regs = [0usize; 32];` block.
- The buggy cfg form described in the issue body (`any(not(x86_64), not(aarch64), miri)`) is **not** present in current sources.

This matches the 2026-03-26 in-file analysis: the issue text described an older cfg; current code matches the intended remediation.
