...
Applied the fix as suggested in the issue - always record SATB OLD values in `write()` and `lock()` methods, regardless of `incremental_active` state.

Changes made to `crates/rudo-gc/src/sync.rs`:
- `GcRwLock::write()` (line 292): Changed `record_satb_old_values_with_state(&*guard, incremental_active)` to `record_satb_old_values_with_state(&*guard, true)`
- `GcRwLock::try_write()` (line 336): Same change
- `GcMutex::lock()` (line 602): Same change
- `GcMutex::try_lock()` (line 642): Same change

This ensures that when `GcRwLockWriteGuard::drop()` or `GcMutexGuard::drop()` is called, the OLD values captured at lock acquisition time are preserved in the SATB buffer, even if `incremental_active` was `false` when the lock was acquired.
