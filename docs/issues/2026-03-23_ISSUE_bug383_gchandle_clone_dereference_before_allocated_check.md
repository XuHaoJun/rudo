# [Bug]: GcHandle::clone dereferences before is_allocated check - TOCTOU type confusion
...
| **Reproducibility (復現難度)** | High | Requires concurrent lazy sweep and handle clone |

---

## 🧩 受影響的組件與環境 (Affected Component & Environment)
- **Component:** `GcHandle::clone` in `handles/cross_thread.rs`
- **OS / Architecture:** All
- **Rust Version:** 1.75+
...
The `gc_box` pointer should be validated (via `is_allocated` check) BEFORE being dereferenced to read any fields.

### 實際行為 (Actual Behavior)

In `cross_thread.rs:615-629`, `GcHandle::clone()`:

1. **Line 615**: `let gc_box = &*self.ptr.as_ptr();` - Dereferences pointer to get `GcBox` reference
2. **Lines 616-621**: Reads `has_dead_flag()`, `dropping_state()`, `is_under_construction()` from `gc_box`
3. **Lines 623-629**: `is_allocated` check happens AFTER the dereference and field reads

**Problem**: If slot is swept and reused between line 615 and lines 623-629, we read fields from the NEW object in the reused slot, not the original object. This is a type confusion vulnerability.

---

## 🔬 根本原因分析 (Root Cause Analysis)

**File:** `handles/cross_thread.rs:615-629`

```rust
fn clone(&self) -> Self {
    // ...
    unsafe {
        let gc_box = &*self.ptr.as_ptr();  // LINE 615 - DEREFERENCE FIRST!
        assert!(
        ...
        if let Some(idx) = crate::heap::ptr_to_object_index(self.ptr.as_ptr() as *const u8)
        {
            ...
            assert!(
                (*header.as_ptr()).is_allocated(idx),  // AFTER dereference!
...
4. Lines 616-621 read B's fields (type confusion!)
5. Lines 623-629: `is_allocated` check passes (slot is allocated with B)
6. Generation check (lines 634-642) passes because we read B's generation twice
...
Move the `is_allocated` check BEFORE the first dereference of `gc_box`:
...
This matches the pattern used in `resolve_impl` (bug382 fix) and prevents the TOCTOU between dereference and validation.
...
**Note**: Same bug pattern as bug382 (fixed in `resolve_impl`) exists in:
- `try_resolve_impl` (lines 354-361)
- `downgrade` (lines 471-481 and 518-527)
- `clone` (lines 615-629)