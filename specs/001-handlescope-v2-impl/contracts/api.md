...
```rust
impl ThreadControlBlock {
    pub fn register_async_scope(&self, scope: &AsyncHandleScope);
}
```

**Postconditions**:
- Adds scope to async_scopes list
...
```rust
impl ThreadControlBlock {
    pub fn unregister_async_scope(&self, scope: &AsyncHandleScope);
}
```

**Postconditions**:
- Removes scope from async_scopes list
