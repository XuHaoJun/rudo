...
fn bench_sustained_60fps(c: &mut Criterion) {
    ...
    c.bench_function("sustained_60fps", |b| {
        b.iter(|| {
            for _ in 0..60 {
                state.reactive_update(updates_per_frame);
                black_box(());
            }
        });
    });
